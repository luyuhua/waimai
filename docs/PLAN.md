...
**状态**：✅ 已完成（2026/06/06，用户手动登录，确认 iframe 结构）
...
| 5 | 出餐规则与自动出餐 | 点击函数已有，规则引擎和端到端验证待完成 | 需和 Step 6 合并为一步 |
| 6 | ~~定时出餐策略~~ | 已合并到 Step 5 | - |
...
| 2 | 美团商家版登录 | ✅ | 2026/06/06 | 用户手动登录，确认 iframe 结构 |
| 3 | 订单数据提取 | ✅ | 2026/06/07 | 详见 §3.5 |
| 4 | 新订单监控 | ✅ | 2026/06/07 | 5秒轮询 + 状态变化追踪 |
| 5 | 出餐规则与自动出餐 | ⏳ | - | 规则引擎 + 端到端验证，待开发 |
| 7 | 异常处理 | ⏳ | - | 调研阶段 |
| 8 | 产品选型决策 | ⏳ | - | 需用户参与决策 |
...
**当前待执行**：Step 5 — 出餐规则与自动出餐

**具体任务**：
1. 设计出餐规则配置结构（策略类型、延迟时间、倒计时阈值等）
2. 在 `monitorOrders` 中集成规则引擎，实现监听 → 规则匹配 → 自动点击的完整链路
3. 处理点击后确认弹窗（如有）
4. 端到端验证：真实订单走完整个流程
5. 异常处理（按钮找不到、重复点击防护等）

**已有基础**：
- `getCardButtons()` 可正确找到出餐按钮（包括 `<div>` 类型）
- `clickOrderButton()` 可点击指定订单的指定按钮
- `cookRemainingTime` / `suggestedCookTime` 数据已可提取
- `status` + `riderStatus` 双维度状态可判断

**暂不实现**：
- 顾客实付金额（hover tooltip 提取，详见 §3.5.9）
